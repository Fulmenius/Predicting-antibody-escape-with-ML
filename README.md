...
## Results
Contrary to our expectations, the vector representation not only didn't increase the average ROC/AUC score of the model, but decreased it. For n_epochs = 20 and for all antibodies the decrease was statistically insignificant according Welch's $t$-test after applying the Bonferroni correction. However, the variance in prediction quality was reduced dramatically, making the model much more reliable. Comparison of results for $5$, $10$ and $20$ epochs indicates that the embedding model overfitting is not likely to be the case of the prediction quality decrease in the model with embedding, since the difference in average quality of prediction between one-hot-using and embedding-using CNN does not change significantly. However underfitting might theoretically be the case for the embedding using model.
